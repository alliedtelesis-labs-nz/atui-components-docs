...
(function() {
		  ...
		  var freeGlobal = typeof moment.commonjsGlobal == 'object' && moment.commonjsGlobal && moment.commonjsGlobal.Object === Object && moment.commonjsGlobal;
...
exports.TimeExtraOptions = void 0;
(function (TimeExtraOptions) {
    TimeExtraOptions["ALL"] = "ALL";
})(exports.TimeExtraOptions || (exports.TimeExtraOptions = {}));
...
class TimeDateUtil {
    ...
    static convertSecondsToUnit(seconds, unit, decimalPlaces, roundUp = true) {
        const momentSeconds = moment.hooks.duration(seconds, exports.Duration[exports.TimeUnit.SECONDS]);
        let secondsInUnit = seconds;
        switch (unit) {
            case exports.TimeUnit.SECONDS:
                return secondsInUnit;
            case exports.TimeUnit.MINUTES:
                secondsInUnit = momentSeconds.asMinutes();
                break;
            case exports.TimeUnit.HOURS:
                secondsInUnit = momentSeconds.asHours();
                break;
            case exports.TimeUnit.DAYS:
        ...
        return roundUp
            ? lodashExports.round(secondsInUnit, decimalPlaces) // Round up
            : lodashExports.floor(secondsInUnit, decimalPlaces); // Round down
    }
    // convertToSeconds: Convert time value to seconds if it isn't already in this form
    static convertToSeconds(time) {
        const valueInUnit = moment.hooks.duration(time.value, exports.Duration[time.unit]);
        return valueInUnit.asSeconds();
    }
    static getSecondsAgoFromDate(date) {
        return moment.hooks(new Date()).diff(date, exports.Duration.SECONDS);
    }
    static getDateFromRelativeTime(time) {
        ...
        return moment.hooks().subtract(time.value, exports.Duration[time.unit]).toDate();
    }
    /**
    ...
    static getRelativeDateRange(time) {
        ...
        const startDate = TimeDateUtil.floorDateByTimeUnit(fullStartDate, exports.Duration.MINUTES);
        return { startDate, endDate: new Date() };
    }
    static getAbsoluteDateRange(time) {
        return {
            startDate: moment.hooks(time.from).toDate(),
            endDate: moment.hooks(time.to).toDate(),
        };
    }
    ...
    static getDateYearsAgo(years, originalDate) {
        return new Date(lodashExports.clone(originalDate).setFullYear(new Date().getFullYear() - years));
    }
    static getDateMonthsAgo(months, originalDate) {
        return new Date(lodashExports.clone(originalDate).setMonth(new Date().getMonth() - months));
    }
    static getCurrentOrDefaultUnit(currentUnit, units) {
        ...
        else if (!lodashExports.isEmpty(units)) {
            return units[0];
        }
    }
    /**
    ...
    static getDataPointIntervalFor30SecPoller(startDate, endDate) {
        const diffSeconds = moment.hooks(endDate).diff(startDate, exports.Duration.SECONDS);
    ...
    static shiftDateByUnit(date, amount, unit) {
        return moment.hooks(date).add(amount, unit).toDate();
    }
    /**
    ...
    static floorDateByTimeUnit(date, unit) {
        return moment.hooks(date).startOf(unit).toDate();
    }
    /**
    ...
    static ceilingDateByTimeUnit(date, unit) {
        return moment.hooks(date)
            .subtract(1, exports.Duration.SECONDS) // remove one second so square numbers are not rounded up
            .add(1, unit)
            .startOf(unit)
            .toDate();
    }
    ...
    static isSameDateByUnit(date1, date2, unit) {
        return moment.hooks(date1).isSame(date2, unit);
    }
...
//# sourceMappingURL=time-date.util-CvNz3hWU.js.map
