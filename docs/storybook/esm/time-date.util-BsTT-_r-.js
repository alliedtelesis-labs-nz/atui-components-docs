import { c as commonjsGlobal, h as hooks } from './moment-C5Sle45O.js';
...
class TimeDateUtil {
    ...
    static convertSecondsToUnit(seconds, unit, decimalPlaces, roundUp = true) {
        const momentSeconds = hooks.duration(seconds, Duration[TimeUnit.SECONDS]);
    ...
    static convertToSeconds(time) {
        const valueInUnit = hooks.duration(time.value, Duration[time.unit]);
        return valueInUnit.asSeconds();
    }
    static getSecondsAgoFromDate(date) {
        return hooks(new Date()).diff(date, Duration.SECONDS);
    }
    static getDateFromRelativeTime(time) {
        ...
        return hooks().subtract(time.value, Duration[time.unit]).toDate();
    }
    /**
    ...
    static getAbsoluteDateRange(time) {
        return {
            startDate: hooks(time.from).toDate(),
            endDate: hooks(time.to).toDate(),
        };
    }
    ...
    static getDataPointIntervalFor30SecPoller(startDate, endDate) {
        const diffSeconds = hooks(endDate).diff(startDate, Duration.SECONDS);
    ...
    static shiftDateByUnit(date, amount, unit) {
        return hooks(date).add(amount, unit).toDate();
    }
    /**
    ...
    static floorDateByTimeUnit(date, unit) {
        return hooks(date).startOf(unit).toDate();
    }
    /**
    ...
    static isSameDateByUnit(date1, date2, unit) {
        return hooks(date1).isSame(date2, unit);
    }
...
export { Duration as D, MIN_DATE as M, TimeDateUtil as T, TimeUnit as a, TimeRangeDisplay as b, DateFormat as c, TimeExtraOptions as d, TimeRangesInHours as e, lodashExports as l };
//# sourceMappingURL=time-date.util-BsTT-_r-.js.map
