...
function handleTabFocusTrap(event, container) {
    if (event.key !== 'Tab') {
        return;
    }
    const focusableElements = getFocusableElements(container);
...
function createMenuKeydownHandler(options) {
    return (event) => {
...
function focusFirstElement(container, delay = 50) {
    setTimeout(() => {
        const focusableElements = getFocusableElements(container);
        if (focusableElements.length > 0) {
...
export { handleHomeEndNavigation as a, createMenuKeydownHandler as c, focusFirstElement as f, handleArrowNavigation as h };
//# sourceMappingURL=keyboard-navigation-EexUVHDN.js.map
