import { r as registerInstance, a as getElement, h } from './index-LINu0edR.js';
...
const AtToasterComponent = class {
    ...
    setHoverHold(toast, held) {
        if (!toast.pauseOnHover) {
            return;
        }
        this.setHold(toast.id, 'hovered', held);
    }
    pause(timer) {
        if (timer.paused) {
            return;
     ...
     */
    render() {
        return (h("div", { key: '0ae7ea16da08d586e42db8678d73e945855bb87e', class: `at-toaster ${this.position}` }, this.toasts.map((toast) => (h("div", { class: this.classSet(toast), key: toast.id, "data-id": toast.id, role: "status", "aria-live": "polite", onClick: () => this.tapToast(toast), onMouseEnter: () => this.setHoverHold(toast, true), onMouseLeave: () => this.setHoverHold(toast, false) }, h("at-message", { type: toast.type, message_title: toast.title, content: toast.message }, toast.action && (h("at-button", { slot: "actions", type: "secondaryText", size: "sm", "data-name": "toast-action", label: toast.action.label, onClick: (event) => {
