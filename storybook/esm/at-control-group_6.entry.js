import { r as registerInstance, h, H as Host, c as createEvent, a as getElement } from './index-B-1zT4Ec.js';
...
const AtTableFilters = class {
    ...
    render() {
        return (this.col_defs && (h(Host, { key: '1b86319026c4273d321036ea249ef20642cc2be4', class: "flex items-start gap-8" }, this.selected.length > 0 && (h("div", { key: '59087eef73a919f426c82b0e8c871d07f5620098', class: "bg-surface-foreground rounded-input flex min-h-[36px] flex-wrap items-end gap-8 p-8" }, this.selected.map((column) => (h("div", { class: "flex items-center gap-2" }, h("at-input", { class: "w-input-sm", label: this.getHeaderName(column.id), prefix: this.getHeaderName(column.id) + ': ', value: column.value, onAtuiChange: (event) => this.filterChangeHandler(event, column.id) }, h("div", { slot: "input-actions" }, h("at-button", { type: "secondaryText", size: "sm", onClick: () => this.clearSingleFilter(column.id), "data-name": `filter-clear-${column.id}` }, h("at-icon", { slot: "icon", name: "cancel" }))))))), h("at-button", { key: 'be5cc2bf9cf8e927d3e3a73eb33cc2253a67b14a', type: "secondaryText", label: "Clear All", onClick: this.clearFilters, "data-name": "filter-clear-all" }))))));
    }
};
