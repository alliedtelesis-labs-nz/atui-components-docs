import { m as moment, h as hooks } from './moment-BMuAbjcg.js';
import { g as getAugmentedNamespace, c as getDefaultExportFromCjs, D as Duration, T as TimeDateUtil, l as lodashExports } from './time-date.util-DLaek6ce.js';
import { a as TimeRangesInHours, D as DateFormat, T as TimeRangeDisplay } from './date-DJyIoUiL.js';
...
class TimeDatePresentationUtil {
    ...
    static getDateLabelMessage(selectedTime, format) {
        if (!selectedTime) {
            ...
            const todayDate = hooks().format(format);
            const previousDate = hooks()
    ...
    static convertTimeRange(timeRange) {
        if (TimeDatePresentationUtil.isCustomRange(timeRange)) {
            return {
                start: hooks(timeRange.custom.from).toDate(),
                end: hooks(timeRange.custom.to).toDate(),
            };
        }
    ...
    static getFormattedDate(date, dateFormat = DateFormat.STANDARD) {
        return date ? hooks(date).format(dateFormat) : '';
    }
        ...
        const translatedDurations = splitDuration.map((duration) => {
            ...
            translation.forEach((trans) => {
                if (duration.includes(' ' + trans.unit)) {
                    ...
                    translatedDuration = lodashExports.replace(duration, unit, ' ' + trans.translation);
                }
            });
    ...
    static getIntervalString(start, end, translations) {
        const msDifference = hooks(end).diff(hooks(start));
        const durationString = (hooks.duration(msDifference, 'milliseconds')).format('d [days], h [hrs], m [min]');
    ...
    static getDayCountFromDateFilter(dateFilter) {
        ...
        const timeDiff = hooks.duration(timeRangeFromDateFilter);
    ...
    static isCustomRange(timeRange) {
        if (timeRange.selected === TimeRangeDisplay.CUSTOM) {
            ...
            return (!lodashExports.isEmpty(customRange) && !!customRange.from && !!customRange.to);
        }
    }
}
...
export { TimeDatePresentationUtil as T };
//# sourceMappingURL=time-date-presentation.util-D5XPK9mo.js.map
