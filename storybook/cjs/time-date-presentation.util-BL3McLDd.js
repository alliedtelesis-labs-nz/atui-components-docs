...
var momentDurationFormatSetup = /*@__PURE__*/timeDate_util.getDefaultExportFromCjs(momentDurationFormatExports);

momentDurationFormatSetup(moment__default.default);
class TimeDatePresentationUtil {
    ...
    static buildDateFromStrings(fromDate, fromTime) {
        ...
        return moment__default.default(`${formattedDate} ${fromTime}`, 'M/D/YYYY hh:mm a').toDate();
    }
    ...
    static getDateLabelMessage(selectedTime, format) {
        if (!selectedTime) {
            ...
            const time = date.TimeRangesInHours[selectedTime.selected];
            const todayDate = moment__default.default().format(format);
            const previousDate = moment__default.default()
    ...
    static convertTimeRange(timeRange) {
        if (TimeDatePresentationUtil.isCustomRange(timeRange)) {
            return {
                start: moment__default.default(timeRange.custom.from).toDate(),
                end: moment__default.default(timeRange.custom.to).toDate(),
            };
        }
        else if (timeRange.selected) {
            return {
                start: moment__default.default()
                    .subtract(date.TimeRangesInHours[timeRange.selected], 'hour')
                    .toDate(),
                end: moment__default.default().toDate(),
            };
        }
    }
    ...
    static getFormattedDate(date$1, dateFormat = date.DateFormat.STANDARD) {
        return date$1 ? moment__default.default(date$1).format(dateFormat) : '';
    }
    ...
    static getIntervalString(start, end, translations) {
        const msDifference = moment__default.default(end).diff(moment__default.default(start));
        const durationString = (moment__default.default.duration(msDifference, 'milliseconds')).format('d [days], h [hrs], m [min]');
    ...
    static getDayCountFromDateFilter(dateFilter) {
        ...
        const timeDiff = moment__default.default.duration(timeRangeFromDateFilter);
        return timeDiff.asDays();
    }
    static getHourCountFromTimeRange(timeRange) {
        if (timeRange.selected == date.TimeRangeDisplay.CUSTOM) {
            if (timeRange.custom) {
            ...
            return date.TimeRangesInHours[timeRange.selected];
        }
    }
...
//# sourceMappingURL=time-date-presentation.util-BL3McLDd.js.map
