...
var customParseFormat = /*@__PURE__*/atTimeDate_util.getDefaultExportFromCjs(customParseFormatExports);
...
atTimeDate_util.dayjs.extend(customParseFormat);
class TimeDatePresentationUtil {
    ...
    static buildDateFromStrings(fromDate, fromTime) {
        return atTimeDate_util.dayjs(`${fromDate} ${fromTime}`, 'YYYY-MM-DD hh:mm a').toDate();
    }
    static getDateLabelMessage(selectedTime, format) {
        if (!selectedTime) {
            ...
            const todayDate = atTimeDate_util.dayjs().format(format);
            const previousDate = atTimeDate_util.dayjs().subtract(time, 'hours').format(format);
    ...
    static convertTimeRange(timeRange) {
        if (TimeDatePresentationUtil.isCustomRange(timeRange)) {
            ...
            return {
                start: atTimeDate_util.dayjs()
                    .subtract(date.TimeRangesInHours[timeRange.selected], 'hours')
                    .toDate(),
                end: atTimeDate_util.dayjs().toDate(),
            };
        }
    }
    ...
    static getIntervalFor30sPollerFromTimeRange(timeRange) {
        ...
        return atTimeDate_util.AtTimeDateUtil.getDataPointIntervalFor30SecPoller(start, end);
    }
    static getFormattedDate(date$1, dateFormat = date.DateFormat.STANDARD) {
        if (!date$1)
            return '';
        const d = atTimeDate_util.dayjs(date$1);
        if (!d.isValid())
            return '';
