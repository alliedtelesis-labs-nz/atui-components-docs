...
const AtLayout = class {
    ...
    render() {
        return index.h(index.Host, { key: 'f1cf18fb7197e713b57d39c01118e8eef58c2d8a' }, this.layoutElement);
    }
};
...
const AtTabTrigger = class {
    ...
    render() {
        const getClassname = classlist.classlist('group/tab text-button no-underline relative box-border flex cursor-pointer items-center gap-4 overflow-hidden rounded-input border border-solid border-transparent !font-medium capitalize transition-[color,background-color,border-color,box-shadow,fill] duration-150 ease-in-out p-8 outline-none focus-visible:z-20 focus-visible:ring focus-visible:ring-active-glow', variantsConfig);
        const classname = getClassname({
            layout: this.layout,
            active: this.is_active,
        });
        return (index.h(index.Host, { key: '6c8be80bde45c20cfee90985a424b3409701aafd', role: "tab", tabindex: "0", "aria-selected": this.is_active, "data-active": this.is_active ? 'true' : 'false', "data-name": `tab-${this.tab_id}`, onKeyDown: (event) => this.handleKeyDown(event), class: `${classname} ${this.fill ? 'flex-1' : undefined}` }, this.tab_title, index.h("slot", { key: '137ed73eef081db3e38622cc0d28bba53e5563ab' })));
    }
};
