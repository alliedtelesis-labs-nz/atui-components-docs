 ...
 */
const DERIVERS = {
    [exports.AvailableCells.TEXT_CELL]: (value, data, params) => {
    ...
    [exports.AvailableCells.TEXT_STATUS_CELL]: (value, _data, params) => {
        if (!value) {
            return '';
        }
        return stringify(params.mapValueToText ? params.mapValueToText(value) : value);
    },
    [exports.AvailableCells.TEXT_WITH_BADGE_CELL]: (value, data, params) => join(params.text ? params.text(data) : value?.text, params.badgeText ? params.badgeText(data) : value?.badgeText),
    [exports.AvailableCells.TITLE_SUBTITLE_CELL]: (_value, data, params) => join(params.title ? params.title(data) : data?.titleSubtitleCell?.title, params.subtitle
        ? params.subtitle(data)
        : data?.titleSubtitleCell?.subtitle),
    [exports.AvailableCells.TITLE_SUBTITLE_DATE_CELL]: (value) => {
     ...
     */
    [exports.AvailableCells.RELATIVE_DATETIME_CELL]: (value, data, params) => {
    ...
    [exports.AvailableCells.CHIP_LIST_CELL]: (_value, data, params) => params.chips ? join(params.chips(data)) : '',
    [exports.AvailableCells.TEXT_WITH_ICON_CELL]: (value, data, params) => stringify(params.text ? params.text(data) : value),
    [exports.AvailableCells.TEXT_WITH_IMAGE_CELL]: (value, data, params) => stringify(params.text ? params.text(data) : value),
    [exports.AvailableCells.PROGRESS_BAR_CELL]: (value, data, params) => {
        if (params.label) {
    ...
    [exports.AvailableCells.EDIT_TEXT_CELL]: (value) => stringify(value),
    [exports.AvailableCells.TOGGLE_CELL]: (_value, data, params) => stringify(params.label ? params.label(data) : data?.toggleCell?.label),
    [exports.AvailableCells.MULTI_BTN_CELL]: (_value, _data, params) => {
        const buttons = typeof params.buttons === 'function' ? [] : params.buttons;
        return Array.isArray(buttons)
            ? join(buttons.map((button) => button?.value))
            : '';
    },
    [exports.AvailableCells.COLOR_STATUS_CELL]: NO_VISIBLE_TEXT,
    [exports.AvailableCells.HEALTH_DOT_CELL]: NO_VISIBLE_TEXT,
    [exports.AvailableCells.CHECKBOX_CELL]: NO_VISIBLE_TEXT,
    [exports.AvailableCells.MENU_CELL]: NO_VISIBLE_TEXT,
};
/**
