...
var require$$0 = /*@__PURE__*/timeDate_util.getAugmentedNamespace(moment.moment);
...
class TimeDatePresentationUtil {
    ...
    static getDateLabelMessage(selectedTime, format) {
        if (!selectedTime) {
            ...
            const todayDate = moment.hooks().format(format);
            const previousDate = moment.hooks()
    ...
    static convertTimeRange(timeRange) {
        if (TimeDatePresentationUtil.isCustomRange(timeRange)) {
            return {
                start: moment.hooks(timeRange.custom.from).toDate(),
                end: moment.hooks(timeRange.custom.to).toDate(),
            };
        }
    ...
    static getFormattedDate(date, dateFormat = timeDate_util.DateFormat.STANDARD) {
        return date ? moment.hooks(date).format(dateFormat) : '';
    }
    ...
    static getIntervalString(start, end, translations) {
        const msDifference = moment.hooks(end).diff(moment.hooks(start));
        const durationString = (moment.hooks.duration(msDifference, 'milliseconds')).format('d [days], h [hrs], m [min]');
    ...
    static getDayCountFromDateFilter(dateFilter) {
        ...
        const timeDiff = moment.hooks.duration(timeRangeFromDateFilter);
...
//# sourceMappingURL=time-date-presentation.util-CNDeyeHs.js.map
