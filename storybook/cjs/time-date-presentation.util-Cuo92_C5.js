...
var customParseFormat = /*@__PURE__*/timeDate_util.getDefaultExportFromCjs(customParseFormatExports);
...
timeDate_util.dayjs.extend(customParseFormat);
class TimeDatePresentationUtil {
    ...
    static buildDateFromStrings(fromDate, fromTime) {
        return timeDate_util.dayjs(`${fromDate} ${fromTime}`, 'YYYY-MM-DD hh:mm a').toDate();
    }
    static getDateLabelMessage(selectedTime, format) {
        if (!selectedTime) {
            ...
            const todayDate = timeDate_util.dayjs().format(format);
            const previousDate = timeDate_util.dayjs().subtract(time, 'hours').format(format);
    ...
    static getFormattedDate(date$1, dateFormat = date.DateFormat.STANDARD) {
        if (!date$1)
            return '';
        const d = timeDate_util.dayjs(date$1);
        if (!d.isValid())
            return '';
