...
const AtToasterComponent = class {
     ...
     */
    render() {
        return (index.h("div", { key: '6e8d8bd36b0d1391a040c834d32aa4375b46b8e1', class: `at-toaster ${this.position}` }, this.toasts.map((toast) => (index.h("div", { class: this.classSet(toast), key: toast.id, "data-id": toast.id, role: "status", "aria-live": "polite", onClick: () => this.tapToast(toast), onMouseEnter: () => this.setHold(toast.id, 'hovered', true), onMouseLeave: () => this.setHold(toast.id, 'hovered', false) }, index.h("at-message", { type: toast.type, message_title: toast.title, content: toast.message }, toast.action && (index.h("at-button", { slot: "actions", type: "secondaryText", size: "sm", "data-name": "toast-action", label: toast.action.label, onClick: (event) => {
            ...
            } }, index.h("at-icon", { slot: "icon", name: "close" })))), this.hasCountdown(toast) && (index.h("div", { class: "at-toast-progress", "data-name": "toast-progress", "aria-hidden": "true" }, index.h("div", { class: `at-toast-progress-fill ${toast.type}`, ref: (el) => this.attachBar(toast.id, el) }))))))));
    }
};
