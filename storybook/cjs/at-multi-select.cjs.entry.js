...
const AtMultiSelectComponent = class {
    ...
    render() {
        return (index.h(index.Host, { key: '1eef61dbce4f24c0fcc97c29a66f1cb099905569', class: "group/select", onFocusout: async (event) => {
            ...
            } }, this.renderLabel(), index.h("at-menu", { key: '107f74b96f4a1e2ef27ff574d9ba041bf628e417', ref: (el) => (this.menuRef = el), trigger: "click", align: "start", width: this.parentWidth, max_height: this.menu_max_height, role: "presentation", autoclose: false, disabled: this.disabled || this.readonly, onAtuiMenuStateChange: (event) => this.updateIsOpenState(event) }, this.renderInput(), !this.disabled && !this.readonly
            ? this.renderOptions()
            : null), index.h("div", { key: '76c67214f549e670d405f1583ccc85a29b12d858' }, this.error_text && this.invalid && (index.h("span", { key: 'd1f70ebda87d750d2cc893de187d107a7624e9c5', "data-name": "multi-select-error", class: "text-error" }, this.error_text)))));
    }
    renderLabel() {
    ...
    renderInput() {
        ...
        return (index.h("div", { id: this.inputId, class: classname, slot: "menu-trigger", "data-name": "multi-select-input-container", tabindex: 0, role: "combobox", "aria-haspopup": "listbox", "aria-expanded": this.isOpen ? 'true' : 'false', "aria-controls": this.menuId, "aria-labelledby": this.label ? `${this.menuId}-label` : undefined }, index.h("at-chip-list", { size: "sm", class: "w-full focus-within:ring-0", readonly: this.readonly, disabled: this.disabled, show_clear_all: this.clearable, onAtRemoveChip: (event) => this.handleRemoveChip(event), chips: this.getSelectedLabels(), "data-name": "multi-select-chips-inside" }, this.value.length === 0 && (index.h("div", { class: "text-body text-disabled pointer-events-none select-none" }, this.placeholder))), !this.readonly && !this.disabled && (index.h("div", { class: "user-select-none fill-foreground pointer-events-none absolute right-4 flex items-center bg-transparent p-4", "data-name": "expand-button", role: "presentation", tabindex: -1 }, index.h("at-icon", { class: "fill-foreground", name: this.isOpen ? 'caret_up' : 'caret_down' })))));
    }
    renderOptions() {
