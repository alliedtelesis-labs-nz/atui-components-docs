...
const AtMultiSelectComponent = class {
    ...
    getOptionClassname = classlist.classlist('transition[background-color,color,box-shadow] text-body focus:ring-active-foreground/40 flex w-full cursor-pointer items-center truncate p-8 font-normal duration-300 ease-in-out focus:ring-2 focus:outline-0 focus:ring-inset', inputVariantsConfig);
    get hasMatchingOptions() {
        return this.filteredOptions.length > 0;
    }
    render() {
        return (index.h(index.Host, { key: 'd7c3052bba17f1715bdc8bc32de2816a5355bc1c', class: "group/select", onFocusout: async (event) => {
            ...
            } }, this.renderLabel(), index.h("at-menu", { key: '7d2e021a3627662e4a83d9b78d0435c230444cb0', ref: (el) => (this.menuRef = el), trigger: "click", align: "start", width: this.parentWidth, role: "listbox", autoclose: false, disabled: this.disabled || this.readonly, onAtuiMenuStateChange: (event) => this.updateIsOpenState(event) }, this.renderInput(), !this.disabled || !this.readonly
            ? this.renderOptions()
            : null), index.h("div", { key: 'f405b5d8feb40f8e359eedf5d0503aaf73f5d36e' }, this.error_text && this.invalid && (index.h("span", { key: 'c6df545636313cba8516b45465ed0bf7e964e880', "data-name": "multi-select-error", class: "text-error" }, this.error_text)))));
    }
    renderLabel() {
    ...
    renderOptions() {
        return (index.h("ul", { id: this.menuId, role: "listbox", class: "contents", onKeyDown: async (event) => {
            ...
            }, onClick: (e) => e.stopPropagation(), ref: (el) => (this.searchInputEl = el) }), this.clearable && this.searchText !== '' && (index.h("div", { class: "absolute top-4 right-4" }, index.h("at-button", { class: "m-2", size: "sm", icon: "cancel", type: "secondaryText", onClick: (event) => {
                event.stopPropagation();
                this.searchText = '';
            ...
            !(this.options && this.options.length > 0
                ...
                : this.hasMatchingElOptions) && (index.h("div", { "data-name": "no-results-found", class: "text-body text-light w-full bg-white px-16 py-8" }, this.translations?.ATUI?.NO_RESULTS_FOUND ||
            'No results found'))));
    }
    renderGroupedOption(option) {
        ...
        return (index.h("at-select-group", { label: option.value }, option.children.map((child) => (index.h("li", { role: "option", "data-name": "multi-select-option", "aria-selected": this.value.includes(child.key), tabIndex: 0, class: this.getOptionClassname({
                active: this.value.includes(child.key),
            }), onClick: () => this.handleChange(child.key) }, child.value)))));
    }
    renderOption(option) {
        return (index.h("at-select-option", { value: option.value, is_active: this.value.includes(option.value), "data-name": "multi-select-option", onAtuiClick: () => this.handleChange(option.value) }));
    }
