...
const AtSearchTable = class {
    constructor(hostRef) {
        ...
        this.atExternalFiltersChange = index.createEvent(this, "atExternalFiltersChange", 7);
        this.atSelectionChange = index.createEvent(this, "atSelectionChange", 7);
    }
    /**
    ...
    get selectionMessage() {
        ...
        if (this.selectionScope === 'all-matching') {
            return this.excludedIds.size
                ? translation.interpolate(selection.ALL_MATCHING_SELECTED_EXCLUDED ?? '', {
                    count,
                    total,
                    noun,
                })
                : translation.interpolate(selection.ALL_MATCHING_SELECTED ?? '', {
                    total: count,
                    noun,
                });
        }
        ...
        if (wholePageSelected && count === page.length) {
            return translation.interpolate(selection.PAGE_SELECTED ?? '', { count, noun });
        }
        const selectedOnPage = page.filter((row) => this.selectedIds.has(this.rowId(row))).length;
        return translation.interpolate((selectedOnPage < count
    ...
    renderSelectionBar() {
        ...
        if (!this.selectionEnabled) {
            return (index.h("div", { class: "selection-bar selection-bar--passive" }, index.h("slot", { name: "multi-select-actions" }), index.h("slot", { name: "floating-multi-select-actions" })));
        }
        return (index.h("div", { class: `selection-bar flex items-center gap-12 ${floating
                ? 'selection-bar--floating border-input bg-surface-background rounded-lg border p-12'
                : 'selection-bar--inline mb-8 py-8'}`, popover: floating ? 'manual' : undefined, ref: (el) => (this.selectionBarEl = el), role: "region", "aria-live": "polite", "data-name": "selection-bar" }, index.h("div", { class: "flex w-full flex-wrap items-center gap-12" }, index.h("span", { class: "flex flex-wrap items-center gap-8" }, index.h("span", { "data-name": "selection-bar-text" }, this.selectionMessage), this.canExpandSelection && (index.h("at-button", { type: "primaryText", size: "sm", "data-name": "select-all-matching", label: translation.interpolate(selection.SELECT_ALL_MATCHING ?? '', { total: this.table_data?.total ?? 0 }), onAtuiClick: () => this.expandSelectionToAllMatching() })), this.canReduceSelectionToPage && (index.h("at-button", { type: "primaryText", size: "sm", "data-name": "select-page-only", label: selection.SELECT_PAGE_ONLY ??
                'Select all in the current page', onAtuiClick: () => this.reduceSelectionToPage() }))), index.h("div", { class: "flex items-center gap-8" }, index.h("slot", { name: "multi-select-actions" }), index.h("slot", { name: "floating-multi-select-actions" }), index.h("at-button", { type: "secondaryText", "data-name": "clear-selection", size: "sm", label: this.translations?.ATUI?.CLEAR_SELECTION ??
    ...
    handleFilterChange(event) {
        ...
        if (externalKeys.size) {
            const survivors = new Set(filterTree_util.flattenFilterConditions(tree).map((condition) => this.conditionKey(condition)));
        ...
        const internalTree = this.pruneConditions(tree, filterTree_util.flattenFilterConditions(tree).filter((condition) => externalKeys.has(this.conditionKey(condition))));
        if (!internalTree || filterTree_util.countFilterConditions(internalTree) === 0) {
    ...
    pruneConditions(group, conditions) {
        ...
        const prune = (node) => ({
            ...
            children: node.children
                .map((child) => (filterTree_util.isFilterGroup(child) ? prune(child) : child))
                .filter((child) => filterTree_util.isFilterGroup(child)
    ...
    mergeFilterTrees() {
        ...
        if (!external || filterTree_util.countFilterConditions(external) === 0) {
            return this.selectedFilters;
        }
        if (filterTree_util.countFilterConditions(this.selectedFilters) === 0) {
            return external;
        }
        return {
            id: 'root',
    ...
    externalConditions() {
        return this.search_filters
            ? filterTree_util.flattenFilterConditions(this.search_filters)
            : [];
    }
    ...
    updateActiveFilters() {
        ...
        filterTree_util.flattenFilterConditions(this.activeFilterTree).forEach((filter) => {
            if (filter.value) {
    ...
    render() {
        return (index.h(index.Host, { key: 'f4c6781c3b4fa6a7a47cb1587e51ffc71e471e8b', class: this.server_side_mode ? 'is-loading' : '' }, index.h("at-table-actions", { key: 'dc17ba7f3f7967f6927b195192cf2298d44047db', ag_grid: this.agGrid }, index.h("at-control-group", { key: '907e8a039ce84b74a73cbc0da42efb8a17137084', slot: "search" }, this.shouldShowTableFilters && (index.h("at-table-filter-menu", { key: 'bd7e190fdb8178780e546bf8b3ff20d5a9d22ccb', ref: (el) => (this.filterMenuEl =
                el), col_defs: this.col_defs, filters: this.selectedFilters, onAtChange: (event) => this.handleFilterChange(event) })), index.h("at-search", { key: '19249617b2a0935d04ebc8ac9009cd808b23e6b5', class: "w-input-md", info_text: this.searchInfoTooltip, placeholder: this.translations.ATUI.TABLE.SEARCH_BY_KEYWORD, onAtChange: (event) => this.handleSearchChange(event) })), index.h("div", { key: 'c7e780290ddaff1a27051c6886ae3c2f2daffb9e', class: "contents", slot: "filter-bar" }, index.h("slot", { key: '14824d61b76da5cea343ebd62c17883e4b100f1d', name: "filter-bar" })), this.hasDisplayableFilters && (index.h("at-table-filters", { key: '8ba5ef9c96cb0a31ead0124270c2146d2186942b', slot: "filters", filters: this.chipFilterTree(), onAtChange: (event) => this.handleFilterChange(event), onAtFilterClick: () => this.filterMenuEl?.openMenu() })), this.show_reload_button && (index.h("at-reload-button", { key: 'bae3e39a9f4d248317be32dc98c2ba3cfe3223e5', slot: "reload-button", has_updates: this.has_updates, onAtuiReload: (event) => {
            ...
            } })), this.show_export_menu && (index.h("at-table-export-menu", { key: '7f20d9437a9e0424822036872322ab481657c384', slot: "export-menu", show_csv: this.show_csv_export, show_pdf: this.show_pdf_export, onAtChange: (event) => this.handleExport(event) })), this.shouldShowColumnManager && (index.h("at-column-manager", { key: '8a9e6f91bc6fd0541b98973e21c638c1b6bb6d1c', slot: "column-manager", col_defs: this.col_defs, onAtChange: (event) => this.handleColumnChange(event) })), index.h("div", { key: 'edd3045df3a2d29952fc877da3ea2fd6c706e782', slot: "leading-actions" }, index.h("slot", { key: 'b34715ea971fbd8e0a1db2726562937d111bedd7', name: "leading-actions" })), index.h("div", { key: '69da4d150647fcb6dffee3b1480da79f9127aabd', slot: "actions" }, index.h("slot", { key: '0e1e90269b830e15cda7734be2a93a649f7c93c5', name: "actions" }))), this.renderSelectionBar(), index.h("div", { key: '8b8c935f5c106886f812da9130c5123b09e15f52', class: "relative" }, index.h("at-table", { key: '3e24f2ae6b18491e356103d4edf98136568b9b22', ref: (el) => (this.tableEl = el), table_data: this.table_data, col_defs: this.gridColDefs, row_id_field: this.row_id_field, page_size: this.server_side_mode
                ? this.pageSize
                : this.page_size, use_custom_pagination: this.server_side_mode || this.use_custom_pagination, use_custom_sorting: this.server_side_mode, auto_size_columns: this.auto_size_columns, can_auto_init: false, onAtColumnVisibilityChange: (event) => this.syncColumnVisibility(event) }), this.server_side_mode && (index.h("div", { key: '24154275c62ae8a29723d2f534160ea86d46e54d', class: `loading-overlay bg-surface-foreground/80 absolute inset-0 z-10 items-center justify-center py-120 ${this.showLoadingOverlay ? 'is-visible' : ''}` }, index.h("div", { key: 'd7b6ce2526321d3008c788b27a94c1068ad5d1e0', class: "flex items-center" }, index.h("at-loading", { key: 'e49587775c3c43f820ec4f0c15b2fb0933b3ac75', class: "relative mr-8", size: "sm", "data-name": "placeholder-spinner" }), index.h("span", { key: '4dfa077477cb8fd9f352ef57f3f0ca8208950e53', class: "text-secondary text-sm font-medium", "data-name": "placeholder-title" }, this.translations?.ATUI?.TABLE
            ?.LOADING_DATA)))), this.server_side_mode && (index.h("div", { key: '942aa2daddd6f5e036e66dcd9a8d425e71e4e206', class: `no-data-overlay absolute inset-0 z-10 flex-col items-center justify-center gap-8 py-120 ${!this.is_loading && this.hasNoData ? 'is-visible' : ''}` }, index.h("at-icon", { key: '4f4edcbd1f92958b545dbf63ae1f643f5534b2bc', class: "fill-slate-300", name: this.hasActiveSearch
                ? 'search'
                : 'data_table', size: "sm", "data-name": "no-data-icon" }), index.h("span", { key: '10d33e4c7751fe1db2df7e60962ac0a0e5bfa939', class: "text-secondary text-sm font-medium", "data-name": "no-data-title" }, this.hasActiveSearch
            ? this.translations?.ATUI?.NO_RESULTS_FOUND
            : (this.no_data_message ??
                this.translations?.ATUI?.TABLE?.NO_DATA))))), this.server_side_mode && (index.h("at-table-pagination", { key: 'dc829e82ce02e5ea01ab0fcc5c9c25457c3e083d', current_page: this.currentPage, num_pages: this.totalPages, page_size: this.pageSize, page_size_options: this.page_size_options, onAtChange: (event) => this.handlePageChange(event), onAtPageSizeChange: (event) => this.handlePageSizeChange(event) }))));
    }
