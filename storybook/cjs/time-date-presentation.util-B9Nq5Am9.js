...
var moment$1 = require('./moment-DRlRqWvO.js');
var moment = require('./moment-w2EYXu2o.js');
var timeDate_util = require('./time-date.util-CvNz3hWU.js');

var momentDurationFormat$1 = {exports: {}};
 ...
 */
var momentDurationFormat = momentDurationFormat$1.exports;

var hasRequiredMomentDurationFormat;

function requireMomentDurationFormat () {
	if (hasRequiredMomentDurationFormat) return momentDurationFormat$1.exports;
	hasRequiredMomentDurationFormat = 1;
	(function (module, exports) {
		(function (root, factory) {
		    {
		        ...
		        try {
		            module.exports = factory(moment.require$$0);
		        } catch (e) {
		...
		})(momentDurationFormat, function (moment) {
	...
	} (momentDurationFormat$1));
	return momentDurationFormat$1.exports;
}

var momentDurationFormatExports = requireMomentDurationFormat();
var momentDurationFormatSetup = /*@__PURE__*/moment$1.getDefaultExportFromCjs(momentDurationFormatExports);

momentDurationFormatSetup(moment$1.hooks);
class TimeDatePresentationUtil {
    static buildDateFromStrings(fromDate, fromTime) {
        const formattedDate = this.dateBuilder(fromDate);
        return moment$1.hooks(`${formattedDate} ${fromTime}`, 'M/D/YYYY hh:mm a').toDate();
    }
    ...
    static getDateLabelMessage(selectedTime, format) {
        if (!selectedTime) {
            ...
            const time = timeDate_util.TimeRangesInHours[selectedTime.selected];
            const todayDate = moment$1.hooks().format(format);
            const previousDate = moment$1.hooks()
                .subtract(time, timeDate_util.Duration.HOURS)
                .format(format);
    ...
    static getCustomFromAndToDate(timeRange) {
        ...
        if (TimeDatePresentationUtil.isCustomRange(timeRange)) {
            ...
            const fromDate = moment$1.hooks(from).toDate();
            const toDate = moment$1.hooks(to).toDate();
    ...
    static convertTimeRange(timeRange) {
        if (TimeDatePresentationUtil.isCustomRange(timeRange)) {
            return {
                start: moment$1.hooks(timeRange.custom.from).toDate(),
                end: moment$1.hooks(timeRange.custom.to).toDate(),
            };
        }
        else if (timeRange.selected) {
            return {
                start: moment$1.hooks()
                    .subtract(timeDate_util.TimeRangesInHours[timeRange.selected], 'hour')
                    .toDate(),
                end: moment$1.hooks().toDate(),
            };
        }
    }
    ...
    static getIntervalFor30sPollerFromTimeRange(timeRange) {
        ...
        return timeDate_util.TimeDateUtil.getDataPointIntervalFor30SecPoller(start, end);
    }
    static getFormattedDate(date, dateFormat = timeDate_util.DateFormat.STANDARD) {
        return date ? moment$1.hooks(date).format(dateFormat) : '';
    }
        ...
        const translatedDurations = splitDuration.map((duration) => {
            ...
            translation.forEach((trans) => {
                if (duration.includes(' ' + trans.unit)) {
                    ...
                    translatedDuration = timeDate_util.lodashExports.replace(duration, unit, ' ' + trans.translation);
                }
            });
    ...
    static getIntervalString(start, end, translations) {
        const msDifference = moment$1.hooks(end).diff(moment$1.hooks(start));
        const durationString = (moment$1.hooks.duration(msDifference, 'milliseconds')).format('d [days], h [hrs], m [min]');
    ...
    static getDayCountFromDateFilter(dateFilter) {
        ...
        const timeDiff = moment$1.hooks.duration(timeRangeFromDateFilter);
        return timeDiff.asDays();
    }
    static getHourCountFromTimeRange(timeRange) {
        if (timeRange.selected == timeDate_util.TimeRangeDisplay.CUSTOM) {
            if (timeRange.custom) {
            ...
            return timeDate_util.TimeRangesInHours[timeRange.selected];
        }
    }
    ...
    static isCustomRange(timeRange) {
        if (timeRange.selected === timeDate_util.TimeRangeDisplay.CUSTOM) {
            ...
            return (!timeDate_util.lodashExports.isEmpty(customRange) && !!customRange.from && !!customRange.to);
        }
    }
}
...
//# sourceMappingURL=time-date-presentation.util-B9Nq5Am9.js.map
