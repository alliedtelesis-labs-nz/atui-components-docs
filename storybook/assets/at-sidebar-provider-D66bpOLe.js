import{j as e,M as s}from"./index-DhDa1EDK.js";import{useMDXComponents as r}from"./index-DFATf2CA.js";import{at as i}from"./MermaidMarkdown-CiDTr6z8.js";import"./iframe-Bw5CQFVg.js";import"./index-DGnb9v-d.js";import"./index-DrFu-skq.js";const o=`# at-sidebar-provider

Wraps two or more \`at-sidebar\` panels that share one page (e.g. a left nav plus a right utility
rail) plus an \`at-sidebar-inset\` for the shared content, instead of nesting \`at-sidebar\` standalone
instances inside each other. \`at-sidebar\`'s stylesheet is unscoped, so nested standalone instances
collide on shared selectors (\`.page-content\`, \`.backdrop\`, \`[data-state]\`); the provider keeps the
panels as siblings and coordinates state between them: a single shared backdrop when any panel is a
modal (backdropped, \`mode="over"\`) overlay, and \`at-sidebar-inset\` going inert while one is open.

\`\`\`html
<at-sidebar-provider>
    <at-sidebar side="left" collapsible="icon">
        <at-sidebar-trigger slot="sidebar-header"></at-sidebar-trigger>
        <at-sidebar-menu slot="sidebar-content">...</at-sidebar-menu>
    </at-sidebar>

    <at-sidebar-inset>
        <!-- shared page content, e.g. a router-outlet -->
    </at-sidebar-inset>

    <at-sidebar side="right" mode="push">
        <!-- e.g. a help/chat rail -->
    </at-sidebar>
</at-sidebar-provider>
\`\`\`

Author \`at-sidebar-inset\` before a same-side \`at-sidebar\` rail where a component nested inside the
inset needs to know that rail's width (e.g. a peek panel confining itself beside it) — that
convention is what lets such a component's own registration-order retry actually find the rail.

A single \`at-sidebar\` used standalone (not nested here) needs no changes — see \`at-sidebar\`'s own
docs for that arrangement.
...
`;function a(n){return e.jsxs(e.Fragment,{children:[e.jsx(s,{title:"Components/Sidebar/Dual Sidebar"}),`
`,e.jsx(i,{md:o})]})}function b(n={}){const{wrapper:t}={...r(),...n.components};return t?e.jsx(t,{...n,children:e.jsx(a,{...n})}):a()}export{b as default};
